...
export const PENDING_VERIFICATION_EMAIL_KEY = "dasaiko.pendingVerificationEmail";
export const PASSWORD_RESET_EMAIL_KEY = "dasaiko.passwordResetEmail";
...
export async function register(username: string, email: string, password: string): Promise<RegistrationResponse> {
  const response = await api.post<RegistrationResponse>("/auth/register", { username, email, password });
...
export async function resetPassword(email: string, code: string, newPassword: string): Promise<PasswordResetConfirmResponse> {
  const response = await api.post<PasswordResetConfirmResponse>("/auth/reset-password", {
    email,
    code,
...
export async function getResearchProfile(): Promise<ResearchProfile> {
  const response = await api.get<ResearchProfile>("/profile/me");
  return response.data;
}
...
export async function updateResearchProfile(
  ...
  const response = await api.put<ResearchProfile>("/profile/me", profile);
  return response.data;
}

export function logout(): void {
  localStorage.removeItem("token");
  window.dispatchEvent(new CustomEvent("dasaiko:logout"));
}
