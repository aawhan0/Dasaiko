...
import { recordPaperVisit } from "@/utils/researchActivity";
...
export function KnowledgeLibrary() {
  ...
  return (
    <div
      ...
      onClick={() => {
        if (documentMenu) setDocumentMenu(null);
      }}
    >
      {deleteError && (
        <p role="alert" className="px-2.5 text-[10px] text-red-400">
          {deleteError}
        </p>
      )}
        ...
        {filteredDocuments.length === 0 ? (
          ...
          <div className="space-y-0.5">
            {filteredDocuments.map((doc) => (
              <div key={doc.id} data-tour-document-id={doc.id}>
                <DocumentCard
                  document={doc}
                  isActive={activeDocumentId === doc.id}
                  onClick={() => handleDocumentClick(doc)}
                  onContextMenu={(x, y) => {
                    setDocumentMenu({ document: doc, x, y });
                  }}
                />
              </div>
            ))}
      ...
      {documentMenu && (
        <DocumentMenu
          ...
          onDelete={() => {
            setDeleteError(null);
            setDocumentToDelete(documentMenu.document);
      ...
      {documentToDelete && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          ...
          >
            <h2 id="delete-document-title" className="text-sm font-semibold text-white">
              Delete document?
            </h2>
            <p className="mt-2 text-sm leading-5 text-zinc-400">
              This permanently deletes <span className="text-zinc-200">{documentToDelete.name}</span>,
            ...
            <div className="mt-5 flex justify-end gap-3">
              <button
                ...
                className="rounded-lg px-3 py-2 text-sm text-zinc-400 transition hover:bg-white/[0.04] hover:text-white disabled:opacity-50"
              >
                Cancel
              </button>
              <button
