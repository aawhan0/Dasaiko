...
import {
  LayoutDashboard, Settings, Plus, BookOpen, MessageSquare,
...
export function Sidebar() {
  ...
  const { conversations, activeConversationId, setActiveConversation, sidebarOpen, toggleSidebar } = useWorkspaceStore();
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-white/[0.06] flex-shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[15px] font-bold text-white tracking-tight">Dasaiko</span>
            </div>
            <button
          ...
          <div className="flex gap-1 px-3 py-2 flex-shrink-0">
            {(['library', 'threads'] as const).map((tab) => (
              <button
                key={tab}
              ...
              >
                {tab === 'library' ? <LayoutDashboard className="w-3.5 h-3.5" /> : <MessageSquare className="w-3.5 h-3.5" />}
                {tab}
              </button>
            ))}
          </div>
                  ...
                  {conversations.map((conversation) => (
                    ...
                    >
                      <div className="flex items-start gap-1.5">
                        {conversation.isPinned && <Pin className="w-3 h-3 text-zinc-600 flex-shrink-0 mt-0.5" />}
                        <p className={cn(
                          'text-[13px] font-medium truncate leading-snug',
                          activeConversationId === conversation.id ? 'text-white' : 'text-zinc-300',
                        )}>
                          {conversation.title}
                        </p>
                      </div>
                      <p className="text-[10px] text-zinc-600 mt-0.5 font-mono">
                        {conversation.messageCount} msgs · {formatRelativeDate(conversation.lastActivityAt)}
                      </p>
                    </button>
                  ))}
                </motion.div>
